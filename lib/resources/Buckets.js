...
class Bucket {
   ...
   */
  async upload(bucketName, fileName, data, params = {}) {
    ...
    const s3Client = await this.getS3Client();
    const files = await s3Client.upload(s3params).promise();
    return files;
  }
  ...
  async listFiles(bucketName, params = {}) {
    ...
    const s3Client = await this.getS3Client();
    const files = await s3Client.listObjectsV2(s3Params).promise();
    return files;
  }
